import pytest
from asyncpg.exceptions import UniqueViolationError

from app import crud
from app.db import database
from app.exception import ItemNotFoundException
from app.schemas.item import ItemCreate, ItemUpdate
from app.tests.utils import random_lower_string


@pytest.mark.asyncio
async def test_create_item_success() -> None:
    name = random_lower_string()
    description = random_lower_string()
    item_in = ItemCreate(name=name, description=description)
    item = await crud.item.create(database, item_in)
    assert item.name == name
    assert item.description == description
    assert item.id
    assert isinstance(item.id, int)


@pytest.mark.asyncio
async def test_create_item_fail_item_already_exist() -> None:
    name = random_lower_string()
    description = random_lower_string()
    item_in = ItemCreate(name=name, description=description)
    item = await crud.item.create(database, item_in)

    with pytest.raises(UniqueViolationError):
        item2 = await crud.item.create(database, item_in)
        stored_item = await crud.item.get(database, item.id)

        assert stored_item
        assert item2 is None


@pytest.mark.asyncio
async def test_get_item_success() -> None:
    name = random_lower_string()
    description = random_lower_string()
    item_in = ItemCreate(name=name, description=description)
    item = await crud.item.create(database, item_in)
    stored_item = await crud.item.get(database, item.id)
    assert stored_item
    assert item.id == stored_item.id
    assert item.name == stored_item.name


@pytest.mark.asyncio
async def test_get_item_fail_item_not_found() -> None:
    item_id = 150

    with pytest.raises(ItemNotFoundException):
        stored_item = await crud.item.get(database, item_id)

        assert stored_item is None


@pytest.mark.asyncio
async def test_update_item_success() -> None:
    name = random_lower_string()
    description = random_lower_string()
    item_in = ItemCreate(name=name, description=description)
    item = await crud.item.create(database, item_in)

    new_name = random_lower_string()
    item_in_2 = ItemUpdate(name=new_name)
    updated_item = await crud.item.update(database, item, item_in_2)

    assert updated_item
    assert item.id == updated_item.id
    assert item.name != updated_item.name
    assert updated_item.description == description
    assert updated_item.name == new_name


@pytest.mark.asyncio
async def test_delete_item() -> None:
    name = random_lower_string()
    description = random_lower_string()
    item_in = ItemCreate(name=name, description=description)
    item = await crud.item.create(database, item_in)

    await crud.item.delete(database, item.id)

    with pytest.raises(ItemNotFoundException):
        item2 = await crud.item.get(database, item.id)
        assert item2 is None
